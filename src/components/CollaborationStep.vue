<template>
    ...
    <section class="w-full min-h-[75svh] md:min-h-[100svh] flex flex-col items-center justify-center px-4">
        ...
        <ul class="mt-12 mx-auto list-none p-0 gap-2">
            <li v-for="(step, idx) in steps" :key="idx" v-reveal="{ delay: idx * 90 }"
                ...
                <div class="relative flex flex-col items-center self-center sm:self-stretch">
                    <div
                        ...
                        <Icon :icon="step.icon" class="text-2xl sm:text-4xl leading-none text-white"
                            aria-hidden="true" />
                    </div>
                </div>
...
// One Iconify icon per step, in display order.
const STEP_ICONS = [
...
const steps = computed(() =>
    tm('collaborationStep.steps').map((step, index) => ({ ...step, icon: STEP_ICONS[index] }))
)
</script>
